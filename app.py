...
class InstitutionSettings(db.Model):
    """Store institutional details for report footers"""
    id = db.Column(db.Integer, primary_key=True)
    ...
    address_postal = db.Column(db.String(20), nullable=True)
    phone_primary = db.Column(db.String(30), nullable=True)
    phone_secondary = db.Column(db.String(30), nullable=True)
    tax_registration = db.Column(db.String(50), nullable=True)
    ...
    def get_full_address(self):
        ...
        if self.address_city:
            city_state.append(self.address_city)
        if self.address_state:
            city_state.append(self.address_state)
        if city_state:
            parts.append(', '.join(city_state))
        if self.address_postal:
            parts.append(self.address_postal)
    ...
    def get_phone_display(self):
        ...
        if self.phone_primary:
            phones.append(self.phone_primary)
...
def api_export_report(report_id):
    ...
    # Get institution settings for footer
    inst_settings = InstitutionSettings.query.first()
    
    # Build institution footer HTML
    inst_footer_html = ''
    if inst_settings:
        inst_parts = []
        if inst_settings.institution_name:
            inst_parts.append(f'<strong>{escape(inst_settings.institution_name)}</strong>')
        if inst_settings.get_full_address():
            inst_parts.append(escape(inst_settings.get_full_address()))
        if inst_settings.get_phone_display():
            inst_parts.append(f'Phone: {escape(inst_settings.get_phone_display())}')
        if inst_settings.tax_registration:
            inst_parts.append(f'Tax ID: {escape(inst_settings.tax_registration)}')
        if inst_parts:
            inst_footer_html = '<br>'.join(inst_parts) + '<hr style="margin: 15px 0; border-style: dashed;">'
    ...
    <body>
        ...
        <div class="footer">
            {inst_footer_html}
            <p>Report Generated: {datetime.utcnow().strftime('%m.%d.%Y %H:%M:%S UTC')}</p>
...
@login_required
def setup():
    """Institution setup page for report footer details"""
    import re
    
    if request.method == 'POST':
        try:
            # Get existing or create new settings (singleton pattern)
            inst_settings = InstitutionSettings.query.first()
            if not inst_settings:
                inst_settings = InstitutionSettings()
                db.session.add(inst_settings)
            
            # Validate and sanitize input
            institution_name = request.form.get('institution_name', '').strip()[:255]
            address_street = request.form.get('address_street', '').strip()[:255]
            address_city = request.form.get('address_city', '').strip()[:100]
            address_state = request.form.get('address_state', '').strip()[:100]
            address_country = request.form.get('address_country', '').strip()[:100]
            address_postal = request.form.get('address_postal', '').strip()[:20]
            phone_primary = request.form.get('phone_primary', '').strip()[:30]
            phone_secondary = request.form.get('phone_secondary', '').strip()[:30]
            tax_registration = request.form.get('tax_registration', '').strip()[:50]
            registration_number = request.form.get('registration_number', '').strip()[:50]
            website = request.form.get('website', '').strip()[:255]
            
            # Validate phone number format (optional field)
            phone_pattern = r'^\+?[\d\s\-\(\)]{7,30}$'
            if phone_primary and not re.match(phone_pattern, phone_primary):
                flash('Invalid primary phone format. Use digits, spaces, dashes, or parentheses.', 'error')
                return render_template('setup.html', settings=inst_settings)
            if phone_secondary and not re.match(phone_pattern, phone_secondary):
                flash('Invalid secondary phone format. Use digits, spaces, dashes, or parentheses.', 'error')
                return render_template('setup.html', settings=inst_settings)
            
            # Validate website URL format (optional field)
            if website and not re.match(r'^https?://[^\s<>"{}|\\^`\[\]]+$', website):
                flash('Invalid website URL. Please enter a valid URL starting with http:// or https://', 'error')
                return render_template('setup.html', settings=inst_settings)
            
            # Update settings
            inst_settings.institution_name = institution_name or None
            inst_settings.address_street = address_street or None
            inst_settings.address_city = address_city or None
            inst_settings.address_state = address_state or None
            inst_settings.address_country = address_country or None
            inst_settings.address_postal = address_postal or None
            inst_settings.phone_primary = phone_primary or None
            inst_settings.phone_secondary = phone_secondary or None
            inst_settings.tax_registration = tax_registration or None
            inst_settings.registration_number = registration_number or None
            inst_settings.website = website or None
            
            db.session.commit()
            flash('Institution settings saved successfully!', 'success')
            return redirect(url_for('setup'))
            
        except Exception as e:
            db.session.rollback()
            flash(f'Error saving settings: {str(e)}', 'error')
            return render_template('setup.html', settings=inst_settings if 'inst_settings' in dir() else None)
    
    # GET request - load existing settings
    inst_settings = InstitutionSettings.query.first()
    return render_template('setup.html', settings=inst_settings)


@app.route('/api/institution-settings')
@login_required
def api_institution_settings():
    """Get institution settings as JSON"""
    inst_settings = InstitutionSettings.query.first()
    if inst_settings:
